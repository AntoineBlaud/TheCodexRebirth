...
try:
    ...
    from tenet.trace_analysis.types import TraceMemory
...
logger = logging.getLogger("Tenet.file")
# -----------------------------------------------------------------------------
# Definitions
# -----------------------------------------------------------------------------
...
# Don't go below 65k, otherwrise you will have grave performances issues while loading the trace due to cache
# associated with the segment and other stuff. read_reg use fast_cache, but if segment is too short, the reg
# will not be cached and it will be slow to fetch the reg value.
DEFAULT_SEGMENT_LENGTH = 1000000
...
class TraceFile(object):
    ...
    def __init__(self, filepath, arch=None):
        ...
        # Finalize the searchable memory
        self.searchable_memory.finalize()
    ...
    def _load_header(self, zip_archive):
        ...
        with zip_archive.open("header", "r") as f:
            ...
            self.ip_addrs = array.array(
                type_from_width(self.arch.POINTER_SIZE))
            ...
            self.mask_sizes = [number_of_bits_set(
                mask) * self.arch.POINTER_SIZE for mask in self.masks]
            ...
            for info in firstline.split(","):
                ...
                if name == "base":
                    ...
                    try:
                        ...
                        current_base = int(value, 16)
                    print(f"Base: {current_base:08X}, Slide: {current_base - int(value, 16):08X}")
                    self.slide = current_base - int(value, 16)
                if name == "slide":
                    self.slide = int(value, 16)
                else:
                    f.seek(0)
                    break
    ...
    def _finalize(self):
        ...
        remapped_ip = {ip_map[address]: i for i,
                       address in enumerate(ip_addrs)}
        ...
        remapped_mem = {mem_map[address]: i for i,
                        address in enumerate(mem_addrs)}
    ...
    def _harvest_stats(self, seg):
        ...
        self.min_unique_mem_addr = min(
            self.min_unique_mem_addr, num_unique_mem_addr)
        self.max_unique_mem_addr = max(
            self.max_unique_mem_addr, num_unique_mem_addr)
    ...
    def _finalize_stats(self):
        ...
        self.avg_unique_mem_addr = self.avg_unique_mem_addr // len(
            self.segments)
        ...
        self.num_bytes_unique_mem = len(
            self.mem_addrs) * self.arch.POINTER_SIZE
...
class SearchableMemory(object):
        ...
        for idx, addr, dat in self.data:
            ...
            for i, e in enumerate(dat):
                ...
                if len(interval.bitmap[offset]):
                    ...
                    # never actually lived during one full step, delete
                    if interval.bitmap[offset][-1][0][0] == idx:
                        interval.bitmap[offset].pop()

                interval.bitmap[offset].append(
                    ([idx, max_int], e))  # start_idx, end_idx, value
    ...
    def finalize(self):
        ...
        self.process_data()
        self.fast_string_enum()
        del self.data

    def fast_string_enum(self):
        PS_LENGTH = 1000  # number of max change for a single address
        self.strings = []
        for interval in self.intervals:
            possible_strings = [""] * PS_LENGTH
            possible_strings_idxs = [0] * PS_LENGTH

            for offset in range(len(interval.bitmap)):
                # for each timestamp where this particular address is modified
                for i in range(PS_LENGTH):
                    if i > len(interval.bitmap[offset]) - 1:
                        break
                    e = interval.bitmap[offset][i]
                    curr_str_idx = e[0][0]
                    char = e[1]
                    curr_str = possible_strings[i]
                    possible_strings_idxs[i] = curr_str_idx

                    # append the string to the final list if is valid
                    def _add():
                        curr_str_rep = curr_str.replace("\x00", "")
                        if is_valid_string(curr_str_rep):
                            self.strings.append((curr_str_rep, curr_str_idx))
                        possible_strings[i] = ""

                    # address is not continuous
                    if len(possible_strings[i]) > 80:
                        _add()

                    # check char validity
                    elif char >= 32 and char <= 126:
                        possible_strings[i] += chr(char)

                    # UTF-16
                    elif char == 0 and len(possible_strings[i]) > 0 and possible_strings[i][-1] != "\x00":
                        possible_strings[i] += chr(char)
            ...
            # add the end of the interval exploration , we append the remaining strings
            for i in range(PS_LENGTH):
                p = possible_strings[i].replace("\x00", "")
                if is_valid_string(p):
                    self.strings.append((p, possible_strings_idxs[i]))

        # sort strings by idx
        self.strings.sort(key=lambda x: x[1])
        logger.info(f"Found {len(self.strings)} strings")

    def get_strings_inside_idx_range(self, start_idx, end_idx):
        results = []
        for s in self.strings:
            if s[1] >= start_idx and s[1] <= end_idx:
                results.append(s[0])
            ...
            for data_ptr in range(len(inte.bitmap) - len(pattern) + 1):
                ...
                for i, char in enumerate(pattern):
                    ...
                    for e in inte.bitmap[data_ptr + i]:
                        if e[1] == char:
                            intersect_inter = self.intesect_intervals(
                                [e[0]], candidate_intervals)
                            if len(intersect_inter) > 0:
                                next_candidate_intervals.extend(
                                    intersect_inter)

                    next_candidate_intervals = self.merge_intervals(
                        next_candidate_intervals)
                    ...
                    if i == len(pattern) - 1:
                        results.append(
                            (candidate_intervals[0][0], data_ptr + inte.bounds[0]))
                        if len(results) >= 1000:
                            return results
        return results
...
class TraceSegment(object):
    ...
    def convert_to_relative_index(self, idx):
        return idx - self.base_idx
    ...
    def from_lines(self, lines):
        ...
        MAX_REG_DATA = self.trace.arch.POINTER_SIZE * \
            len(self.trace.arch.REGISTERS) * self.trace.segment_length
    ...
    def get_reg_delta(self, idx):
        ...
        offset = cache_offset + sum([sizes[mask_id]
                                    for mask_id in offset_masks])
        ...
        reg_data = self.reg_data[offset: offset +
                                 (num_regs * self.arch.POINTER_SIZE)]
        ...
        for i, mask_id in enumerate(search_masks):
            ...
            for reg_name in found_names:
                found_registers[reg_name] = (
                    registers[reg_name], (self.base_idx + (start_idx - i)))
    ...
    def load(self, f):
        ...
        reg_mask_type = type_from_width(
            info.reg_mask_length // info.reg_mask_num)
    ...
    def _compute_mem_offsets(self):
        ...
        mem_sets = [(self.read_offsets, self.read_masks),
    ...
    def _process_line(self, line, relative_idx):
        ...
        try:
            # split the line into individual register/memory deltas
            delta = line.split(",")
        except ValueError:
            # FAIL: invalid line
            return False
        ...
        for item in delta:
            ...
            # handle memory r/w/rw access
                ...
                if address < self.arch.MAX_INT_VALUE:
                    self.trace.searchable_memory.record_data_for_search(
                        relative_idx + self.base_idx, address, data)
    ...
    def _process_mem_entry(self, address, data, access_type, relative_idx):
        ...
        for mapped_address, access_mask, access_data in self._map_mem_access(address, data):
            ...
            if access_type == "MR":
                ...
                self._mem_read_info.append(
                    (relative_idx, mapped_address, access_mask))
                self.read_data += access_data
                # self._max_read_size = max(self._max_read_size, data_len)

            # write
            elif access_type == "MW":
                self._mem_write_info.append(
                    (relative_idx, mapped_address, access_mask))
            ...
            # read AND write (eg, inc [rax])
                ...
                raise ValueError(
                    "Unknown field in trace: '%s=...'" % access_type)
    ...
    def _pack_registers(self, registers, relative_idx):
        ...
        fst_cache = self.fast_cache_reg_data
        # append  re_info to the fast cache
        fst_cache[relative_idx] = dict(registers)
        for reg_name, reg_value in fst_cache.get(relative_idx - 1, {}).items():
            if reg_name not in fst_cache[relative_idx]:
                fst_cache[relative_idx][reg_name] = reg_value
        ...
        value_pairs = sorted([(self.arch.REGISTERS.index(name), value)
                             for name, value in registers.items()])
        values = [x[1] for x in value_pairs]
        pack_fmt = "Q" if self.arch.POINTER_SIZE == 8 else "I"
        struct.pack_into(pack_fmt * num_regs, self.reg_data,
                         self._reg_offset, *values)
