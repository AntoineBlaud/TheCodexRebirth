...
class MemoryManager():
    ...
    def hook_code_with_shellcode(self, address, shellcode, shellcode_location, overwrite_instruction_at_address=True , split =False, max_patch_size = 5):
        ...
        margin = 0x100 # for the original code to be copied for jumps, original code, etc ... 

        original_code_at_shellcode_location = idc.get_bytes(shellcode_location, shellcode_size + margin)
        ...
        print(f'Total Shellcode Size 0x{total_shellcode_size:X}, jump_back_address: 0x{jump_back_address:X}')
