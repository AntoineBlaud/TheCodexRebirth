from tenet.tracer.managers import BreakpointManager, MemoryManager, Windows_64_SM, Linux_64_SM
...
class LogicBlockTracer:
    def __init__(self, dctx, arch, model, flog, shellcode_manager):
        ...
        self.memory_manager = MemoryManager(self.cs, self.ks)
    ...
    def init(self):
        ...
        print(f"Hooked address: {hex(hook.address_hooked)} - {hex(hooked_address_end)}")
    ...
    def restore_original_code(self, address, original_code):
        print(f"Restoring original code at {hex(address)}, size: {len(original_code)}")
