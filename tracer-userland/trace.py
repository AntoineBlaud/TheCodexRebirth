...
def read_file_addrs(path: str) -> int:
    addrexpr = re.compile(r'0x[0-9a-fA-F]+')
    with open(os.path.join(path), 'r') as fd:
        return list(map(lambda x: int(x, 16), addrexpr.findall(fd.read())))
...
def do_trace(args, attach):
    ...
    try:
        ...
        try:
            f_addrs = read_file_addrs(args.f_map)
            print(f"Discovered {len(f_addrs)} functions to attach stalkers on")
        except ValueError:
...
def main():
    ...
    parser.add_argument(
        'f_map',
        type=str,
        help='file containing addresses to instrument'
    )
