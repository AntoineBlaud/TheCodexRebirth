...
def patch_different_bytes(segments):
    for seg, (seg_start, seg_end, seg_name, pc, seg_data) in segments.items():
        ...
        differing_indices = [
            i for i, (a, b) in enumerate(zip(existing_bytes, seg_data)) if a != b
        ]
        ...
        while i < len(differing_indices):
            ...
            while (
                i + 1 < len(differing_indices)
                and differing_indices[i + 1] == end_index + 1
            ):
...
def step_and_check_loop():
    ...
    if (
        ...
        and current_address not in jumps
        and current_address not in loop_already_skipped
    ):
        ...
        print(
            f"Identified loop at {hex(current_address)}, j_target={hex(j_target_address)}, j_next={hex(j_next)}"
        )
