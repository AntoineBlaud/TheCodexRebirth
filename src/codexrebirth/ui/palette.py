...
class PluginPalette(object):
    ...
    def warmup(self):
        ...
        print("Could not warmup theme subsystem!")
    ...
    def _validate_theme(self, theme):
        ...
        if not user_fields:
            print("Could not find theme 'fields' definition")
            return False

        # check that all the 'required' fields exist in the given theme
        for field in self._required_fields:
            if field not in user_fields:
                print("Could not find required theme field '%s'" % field)
                return False
    ...
    def _load_theme(self, filepath):
        ...
        # reading file from dsik failed
            ...
            print("Could not open theme file at '%s'" % filepath)
            return False

        # JSON decoding failed
        except JSONDecodeError as e:
            print("Failed to decode theme '%s' to json" % filepath)
            print(" - " + str(e))
            return False

        # do some basic sanity checking on the given theme file
        if not self._validate_theme(theme):
            print("Failed to validate theme '%s'" % filepath)
            return False
        ...
        try:
            ...
            print("Failed to load the plugin user theme\n%s" % e)
            return False
